...
extern int platform_uart_recv( unsigned id, unsigned timer_id, u32 timeout );

static int readchar(void) {
	int foo = platform_uart_recv(0, 0x100, TERM_INPUT_DONT_WAIT);
	return ( foo == -1 ? CHAR_NONE : foo );
}
